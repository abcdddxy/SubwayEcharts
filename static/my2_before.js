    ...
    if(searchst!=-1){
        ...
        success:function(msg){
            ...
            for(var line in msg.upqueue){
                ...
                tb.rows[rownum+1].cells[1].innerHTML = '上行 '+msg.upqueue[line][0];
                tb.rows[rownum+2].cells[1].innerHTML = '下行 '+msg.upqueue[line][1];
            }
            ...
            tb.rows[rownum].cells[1].innerHTML = '出站&nbsp'+msg.downqueue;
            content = ipt + "&nbsp;";
            if(msg.num<100)
    ...
    $.ajax({
        ...
        success:function(msg){
            ...
            for(var line in msg.upqueue){
                ...
                // tb.rows[rownum+2].insertCell(0);
                // tb.rows[rownum+2].insertCell(1);
                tb.rows[rownum].cells[0].innerHTML = temp;
                tb.rows[rownum].cells[1].innerHTML = '上'+msg.upqueue[line][0]+'|下'+msg.upqueue[line][1];
                // tb.rows[rownum].cells[2].innerHTML = msg.upqueue[line][0] + msg.upqueue[line][1];
                // tb.rows[rownum+1].cells[1].innerHTML = '↑'+msg.upqueue[line][0];
                // tb.rows[rownum+2].cells[1].innerHTML = '↓'+msg.upqueue[line][1];
            }
            tb.rows[1].cells[1].innerHTML = lines;
            tb.rows[2].cells[1].innerHTML = msg.num;
            rownum = tb.rows.length;
            ...
            tb.rows[rownum].insertCell(0);
            tb.rows[rownum].insertCell(1); 
            tb.rows[rownum].cells[0].innerHTML = '出站&nbsp'
            tb.rows[rownum].cells[1].innerHTML = msg.downqueue;
